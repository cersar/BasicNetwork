...
def process_predict(probs, confs, boxes, image_size,version,score_threshold=0.5,iou_threshold=0.2,max_boxes_num=10):
    ...
    for c,inds in D.items():
        ...
        probs_cls = np.squeeze(probs_final[inds, ...].reshape((-1, 1)),axis=-1)
...
def show_dection(image_data, probes, confs, boxes_cord,labels,version='yolov1'):
    boxes = yolo_boxes_to_corners(boxes_cord).eval()
    for i in range(len(image_data)):
        result = process_predict(probes[i], confs[i], boxes[i], image_data[i].shape,version)
        start = time.time()
